function captureCamera(n){navigator.mediaDevices.getUserMedia({audio:!0,video:!0}).then(function(t){n(t)}).catch(function(n){alert("Unable to capture your camera. Please check console logs.");console.error(n)})}function stopRecordingCallback(){video.src=video.srcObject=null;video.muted=!1;video.volume=1;video.src=URL.createObjectURL(recorder.getBlob());recorder.camera.stop();recorder.destroy();recorder=null}function getRandomString(){var t,i,n,r;if(window.crypto&&window.crypto.getRandomValues&&navigator.userAgent.indexOf("Safari")===-1){for(t=window.crypto.getRandomValues(new Uint32Array(3)),i="",n=0,r=t.length;n<r;n++)i+=t[n].toString(36);return i}return(Math.random()*(new Date).getTime()).toString(36).replace(/\./g,"")}function getFileName(n){var t=new Date,i=t.getUTCFullYear(),r=t.getUTCMonth(),u=t.getUTCDate();return"RecordRTC-"+i+r+u+"-"+getRandomString()+"."+n}function stopVideo(){document.getElementById("btn-stop-recording").disabled=!0;recorder.stopRecording().then(function(){console.info("stopRecording success 2");var n=recorder.getBlob();video.src=URL.createObjectURL(n);video.play();video.muted=!1;recorder.stream.stop();document.getElementById("btn-start-recording").disabled=!1}).catch(function(n){console.error("stopRecording failure",n)})}var recorder;document.getElementById("btn-start-recording").onclick=function(){this.disabled=!0;captureCamera(function(n){video.muted=!0;video.volume=0;video.srcObject=n;recorder=RecordRTC(n,{type:"video"});recorder.startRecording();recorder.camera=n;document.getElementById("btn-stop-recording").disabled=!1})};document.getElementById("btn-stop-recording").onclick=function(){this.disabled=!0;document.getElementById("btn-start-recording").disabled=!1;recorder.stopRecording(stopRecordingCallback)};var video=document.getElementById("my-preview"),fileExtension="webm",mimeType="video/webm",fileExtension="webm",type="video",recorderType,defaultWidth,defaultHeight,recorder;document.getElementById("btn-start-recording-old").addEventListener("click",function(){this.disabled=!0;navigator.mediaDevices.getUserMedia({audio:!0,video:!0}).then(function(n){setSrcObject(n,video);video.play();video.muted=!0;recorder=new RecordRTCPromisesHandler(n,{mimeType:"video/webm",bitsPerSecond:128e3});recorder.startRecording().then(function(){console.info("Recording video ...")}).catch(function(n){console.error("Cannot start video recording: ",n)});recorder.stream=n;document.getElementById("btn-stop-recording").disabled=!1}).catch(function(n){console.error("Cannot access media devices: ",n)})},!1);document.getElementById("btn-stop-recording").addEventListener("click",function(){this.disabled=!0;stopVideo()},!1);$("#record").on("hidden.bs.modal",function(){stopVideo()});class HttpServices{_handleErrors(n){if(n.ok)return n;throw new Error(n.statusText);}_get(n){return fetch(n).then(n=>this._handleErrors(n)).then(n=>n.json())}_post(n,t){return fetch(n,{headers:{"Content-type":"application/json"},method:"post",body:JSON.stringify(t)}).then(this._handleErrors).then(n=>n.json()).catch(n=>{console.log(n)})}}class QuestionServices{constructor(){this._dao=new QuestionDao}add(n){return this._dao.add(n).then(n=>n).catch(n=>console.log(n))}list(n){return this._dao.list(n).then(n=>n).catch(n=>console.log(n))}}class Arrays{constructor(){throw new Error("This is a Static Class, cannot be intanciate!");}static randomArray(n,t){for(var i=n.length,u,r;0!==i;)r=Math.floor(Math.random()*i),i-=1,u=n[i],n[i]=n[r],n[r]=u;return n.sort((n,t)=>n>t).slice(0,t==0?n.length:t)}}class Question{constructor(n,t,i,r,u){this._description=n;this._tips=t;this._answer=i;this._categoryid=r;this._seconds=u;this._published=!1;Object.freeze(this)}get description(){return this._description}get tips(){return this._tips}get answer(){return this._answer}get seconds(){return this._seconds}}class QuestionDao{constructor(){this._http=new HttpServices;this._baseUrl="/api/Question"}add(n){return new Promise((t,i)=>{this._http._post(this._baseUrl,n).then(n=>{t(n)}).catch(n=>{console.log(n),i("Somethings is wrong")})})}list(n){return new Promise((t,i)=>{this._http._get(this._baseUrl+"/"+n.toString()).then(n=>{t(n)}).catch(n=>{console.log(n),i("Somethings is wrong")})})}}class View{constructor(n){this._elemento=n}template(){throw new Error("Have be implement!");}update(n){this._elemento.innerHTML=this.template(n)}}class QuestionView extends View{constructor(n){super(n)}template(n){return n.length==0?`
            <div id="accordions">
            ...
            `:`
        <div id="accordions">
                    ${n.map(n=>{let t="";return t=n.answer?`<div class="col-6"><button class="btn btn-info text-left" data-toggle="collapse" data-target="#collapse${n.id}" aria-expanded="true" aria-controls="collapse${n.id}">Answer/Tips</button></div>`:"",`
